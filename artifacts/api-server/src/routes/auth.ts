...
// GET /api/auth/user — return fresh user data including plan/credits
router.get("/auth/user", async (req: Request, res: Response) => {
  ...
  // Fetch fresh data from DB (session may be stale on credits)
  const freshUser = await storage.refreshFreeCreditsIfNeeded(req.user.id);
  if (!freshUser) {
    res.json({ isAuthenticated: false, user: null });
    return;
  }
